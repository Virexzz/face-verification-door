...
function App() {
  ...
  const deleteLogItem = async (id) => {
    ...
    try {
      const response = await axios.delete(`${BACKEND_URL}/logs/${id}`);
  ...
  const handleStartFeed = async () => {
    try {
      ...
      setSystemMessage('Failed to initialize engine camera hardware.');
    }
  };
  ...
  const handleStopFeed = async () => {
    try {
      ...
      setSystemMessage('Failed to cleanly shut down engine camera hardware.');
    }
  };
  ...
  const verifyCurrentFace = async () => {
    ...
    try {
      ...
      if (data.authenticated) {
        ...
        setSystemMessage('Unknown identity trace. Choose whether to register this profile.');
        ...
        <div className="layout-left-column">
          ...
          <div className="control-card command-panel">
            ...
            <div className="button-group">
              {!isFeedActive ? (
                <button className="btn btn-start" onClick={handleStartFeed}>
                  Power On Camera Feed
                </button>
              ) : (
                <>
                  ...
                  <button className="btn btn-stop" onClick={handleStopFeed}>
                    Power Off Feed
                  </button>
                </>
              )}
          ...
          <div className="logs-scroller">
            {activityLogs.length === 0 ? (
              <div className="empty-state">
                <p>No transactions registered inside audit database registers.</p>
              </div>
            ) : (
              activityLogs.map((log) => (
                <div key={log.id} className={`log-item-row status-${log.status.toLowerCase()}`}>
                  <div className="log-thumbnail-wrapper">
                    {log.image_filename ? (
                      <img 
                        src={`${SERVER_BASE}/captures/${log.image_filename}`} 
                        alt="Audit Event Snapshot" 
                        className="log-item-thumbnail"
                      />
                    ) : (
                  ...
                  <div className="log-item-details">
                    ...
                    <div className="log-row-bottom">
                      ...
                      <button className="btn-delete-log" onClick={() => deleteLogItem(log.id)}>
                        Remove
                      </button>
                    </div>
                  </div>
      ...
      {showEnrollModal && (
        <FaceEnrollModal 
          ...
          onDeny={() => {
            ...
            setScanStatus('Live Video Active');
            setSystemMessage("Profile registration discarded. Continuing live stream.");
          }}
          onSuccess={(msg) => {
            setShowEnrollModal(false);
            setScanStatus('Live Video Active');
